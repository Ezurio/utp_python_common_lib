...
def program_nrfutil_cmd(file_path: str, serial_number: str, device="", mass_erase=False, unlock=True):
    out = [ ]
    ...
    if NETWORK_CORE_FILE_NAME in Path(file_path).name:
        program_cmd.extend(["--core", NETWORK_CORE])
        recover_device = False

    if recover_device:
        out.append(recover_cmd)

    if mass_erase:
        options.append("chip_erase_mode=ERASE_ALL")
    program_cmd.extend(options)

    out.append(program_cmd)
    out.append(reset_cmd)
...
def program_nrfutil(file_path: str, serial_number: str, device="", mass_erase=False, unlock=True):
    ...
    """
    cmd = program_nrfutil_cmd(file_path, serial_number, device, mass_erase, unlock)
    try:
        for c in cmd:
