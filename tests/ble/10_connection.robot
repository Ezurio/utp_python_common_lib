...
Connect Central
    ...
    Switch Board to Raw REPL    ${primary_board}
    WHILE    $total_time > ${0}
        ${resp1}=    Raw REPL Exec    ${primary_board}    print(found)
        ...
        END
    END
    Switch Board to User REPL    ${primary_board}
...
Check Connection
    ...
    Switch Board to Raw REPL    ${primary_board}
    Switch Board to Raw REPL    ${secondary_board}
    WHILE    $total_time > ${0}
        ${resp1}=    Raw REPL Exec    ${primary_board}    print(current_state)
        ${resp2}=    Raw REPL Exec    ${secondary_board}    print(current_state)
        ${resp1}=    Convert To String    ${resp1}
    ...
    END
    Switch Board to User REPL    ${primary_board}
    Switch Board to User REPL    ${secondary_board}
    IF    ${result} == False    Fail    Failed to connect
